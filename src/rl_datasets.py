...
class BaseTask(abc.ABC):
    ...
    def compute_total_reward(self, response: str, reference: Any) -> dict[str, Any]:
        ...
        return {
            ...
            "reward_info": {"format_reward": fmt_rew, "answer_reward": ans_rew},
        }
...
class CountdownTask(BaseTask):
    ...
    def reward_function(self, response: str, reference: dict[str, Any]) -> float:
        ...
        if not model_ans:
            return 0.0
        if "=" in model_ans:
            model_ans = model_ans.split("=")[0].strip()

        if not re.match(r"^[0-9+\-*/() ]+$", model_ans):
            return 0.0
...
class RLDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {"prefix": prefix, "ids": tokens["input_ids"], "reference": reference}
...
def get_task(task_name: str, tokenizer) -> BaseTask:
    task_map = {"gsm8k": GSM8KTask, "math": MATHTask, "countdown": CountdownTask}
